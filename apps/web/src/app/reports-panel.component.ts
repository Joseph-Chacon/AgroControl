...
type Target = { cropId: string; label: string };
type Report = {
  crop: { name: string; lot: { name: string; farm: { name: string } } };
  ...
  estimatedProfitabilityPercent: string;
  realizedProfitabilityPercent: string;
};

@Component({
  selector: 'ac-reports-panel',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: ` <section class="workspace">
    <h2>Reportes por ciclo de cultivo</h2>
    <form (ngSubmit)="loadReport()">
      <select name="crop" [(ngModel)]="cropId" required>
        ...
        @for (target of targets; track target.cropId) {
          <option [value]="target.cropId">{{ target.label }}</option>
        }</select
      ><input name="startDate" [(ngModel)]="startDate" type="date" aria-label="Desde" /><input
        name="endDate"
        [(ngModel)]="endDate"
        type="date"
        aria-label="Hasta"
...
export class ReportsPanelComponent {
  ...
  constructor(
    ...
    this.http
      .get<
        {
          name: string;
          lots: { name: string; crops: { id: string; name: string; plantedAt: string; plantedPlants: number; isActive: boolean }[] }[];
        }[]
      >('http://localhost:3000/api/v1/agriculture/farms')
      .subscribe((farms) => {
        this.targets = farms.flatMap((f) =>
          f.lots.flatMap((l) =>
            l.crops.map((c) => ({
              cropId: c.id,
              label: `${f.name} · ${l.name} · ${c.name} · Siembra: ${c.plantedAt.slice(0, 10)} · ${c.plantedPlants} matas${c.isActive ? '' : ' · Finalizado'}`,
            })),
          ),
        );
  ...
  loadReport() {
    ...
    const params = new URLSearchParams({ cropId: this.cropId });
    if (this.startDate) params.set('startDate', this.startDate);
    if (this.endDate) params.set('endDate', this.endDate);
    this.http.get<Report>(`http://localhost:3000/api/v1/finance/crop-profitability?${params}`).subscribe((report) => {
      this.report = report;
      this.cdr.detectChanges();
    });
  }

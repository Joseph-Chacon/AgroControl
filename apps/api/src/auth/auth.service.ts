...
export class AuthService {
  ...
  async bootstrapAdmin(dto: BootstrapAdminDto) {
    ...
    if (dto.setupToken !== this.config.getOrThrow<string>('INITIAL_SETUP_TOKEN')) throw new UnauthorizedException('Token de configuración inválido.');
    const passwordHash = await bcrypt.hash(dto.password, 12);
    const user = await this.prisma.$transaction(async (tx) => {
      const role = await tx.role.upsert({ where: { code: 'ADMIN' }, update: {}, create: { code: 'ADMIN', name: 'Administrador' } });
      const created = await tx.user.create({ data: { email: dto.email.toLowerCase(), firstName: dto.firstName, lastName: dto.lastName, passwordHash } });
  ...
  async login(dto: LoginDto) {
    ...
    if (!user || !user.isActive || !(await bcrypt.compare(dto.password, user.passwordHash))) throw new UnauthorizedException('Credenciales inválidas.');

...
type Harvest = {
  ...
  quantity: string;
  crop: Crop;
  items: { grade: string; boxes: number; unitPrice: string | null; total: string | null }[];
};
  ...
  template: `
    <section class="workspace">
      ...
      <form class="harvest-form" (ngSubmit)="save()">
        <select name="crop" [(ngModel)]="cropId" required>
          ...
          @for (crop of crops; track crop.id) {
            <option [value]="crop.id">
              {{ crop.lot.farm.name }} · {{ crop.lot.name }} · {{ crop.name }} · Siembra: {{ crop.plantedAt | slice: 0 : 10 }} ·
              {{ crop.plantedPlants }} matas
            </option>
          }
        </select>
        ...
        <p class="total">
          <strong>{{ hasPendingNewPrice ? 'Valor pendiente de precio' : 'Total de la cosecha: ₡' + total }}</strong>
        </p>
      ...
      @if (harvests.length) {
        <table>
          ...
          <tbody>
            @for (harvest of harvests; track harvest.id) {
              <tr>
                <td>{{ harvest.harvestedAt | slice: 0 : 10 }}</td>
                <td>{{ harvest.crop.lot.farm.name }} · {{ harvest.crop.lot.name }} · {{ harvest.crop.name }}</td>
                <td>{{ harvest.quantity }}</td>
                <td>{{ isPending(harvest) ? 'Precio pendiente' : '₡' + harvestTotal(harvest) }}</td>
                <td>
                  <button type="button" (click)="openPrices(harvest)">{{ isPending(harvest) ? 'Agregar precio' : 'Ver precios' }}</button
                  ><button type="button" (click)="voidHarvest(harvest)">Anular</button>
                </td>
              </tr>
            }
          </tbody>
        ...
        <p class="empty">Aún no hay cosechas registradas.</p>
      }
        ...
        <div class="detail" role="dialog" aria-modal="true" aria-label="Registrar precios de cosecha">
          ...
          <div class="price-lines">
            @for (line of priceLines; track line.grade) {
              <label
                ><span>{{ line.label }} — {{ line.boxes }} cajas</span
                ><input
                  [name]="'update-price-' + line.grade"
...
export class HarvestsPanelComponent {
  ...
  constructor(
    ...
    this.http
      ...
      .subscribe((farms) => {
        this.crops = farms.flatMap((farm) =>
          farm.lots.flatMap((lot) =>
            lot.crops.filter((crop) => crop.isActive).map((crop) => ({ ...crop, lot: { name: lot.name, farm: { name: farm.name } } })),
          ),
        );
  ...
  get hasItems() {
    return this.lines.some((line) => Number(line.boxes) > 0);
  }
  get hasPendingNewPrice() {
    return this.lines.some((line) => Number(line.boxes) > 0 && line.unitPrice === null);
  }
  get hasPrices() {
    return this.priceLines.some((line) => line.unitPrice !== null && line.unitPrice !== undefined);
  }
  get total() {
    return this.lines.reduce((sum, line) => sum + Number(line.boxes || 0) * Number(line.unitPrice || 0), 0);
  }
  lineTotal(line: HarvestLine) {
    return Number(line.boxes || 0) * Number(line.unitPrice || 0);
  }
  harvestTotal(harvest: Harvest) {
    return harvest.items.reduce((sum, item) => sum + Number(item.total || 0), 0);
  }
  isPending(harvest: Harvest) {
    return harvest.items.some((item) => item.unitPrice === null);
  }
  ...
  save() {
    const items = this.lines
      .filter((line) => Number(line.boxes) > 0)
      .map((line) =>
        line.unitPrice === null
          ? { grade: line.grade, boxes: Number(line.boxes) }
          : { grade: line.grade, boxes: Number(line.boxes), unitPrice: Number(line.unitPrice) },
      );
    if (!this.cropId || !items.length) return;
    this.http
      .post('http://localhost:3000/api/v1/finance/harvests', { cropId: this.cropId, harvestedAt: this.harvestedAt, items })
      .subscribe(() => {
  ...
  openPrices(harvest: Harvest) {
    ...
    this.priceLines = harvest.items.map((item) => ({
      grade: item.grade,
      label: this.gradeLabel(item.grade),
      boxes: item.boxes,
  ...
  savePrices() {
    ...
    const items = this.priceLines
      .filter((line) => line.unitPrice !== null && line.unitPrice !== undefined)
      .map((line) => ({ grade: line.grade, unitPrice: Number(line.unitPrice) }));
    if (!items.length) return;
    this.http.patch(`http://localhost:3000/api/v1/finance/harvests/${this.pricing.id}/prices`, { items }).subscribe(() => {
      this.closePrices();
      this.load();
    });
  }
  ...
  gradeLabel(grade: string) {
    return (
      (
        { FIRST: 'Primera', SECOND: 'Segunda', THIRD: 'Tercera', BOLITA: 'Bolita', RAYADO: 'Rayado', ECHADO: 'Echado' } as Record<
          string,
          string
        >
      )[grade] || grade

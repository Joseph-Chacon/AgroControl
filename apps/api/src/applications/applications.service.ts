  ...
  async create(dto: CreateApplicationDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (dto.cropId) {
        ...
        if (crop.isActive) {
          if (!lot.isActive || !lot.farm.isActive) throw new BadRequestException('El cultivo pertenece a una finca o lote inactivo; active la estructura antes de registrar.');
        } else {
          ...
          if (appliedAt > end) throw new BadRequestException('El ciclo está cerrado. Solo se permiten aplicaciones históricas entre la siembra y el retiro de las matas.');
        }
      } else if (!lot.isActive || !lot.farm.isActive) throw new BadRequestException('El lote pertenece a una finca o lote inactivo.');
      const costs: { productId: string; quantity: Prisma.Decimal; averageCost: Prisma.Decimal; totalCost: Prisma.Decimal; before: Prisma.Decimal; after: Prisma.Decimal }[] = [];
      for (const item of dto.items) {
        ...
        costs.push({ productId: item.productId, quantity, averageCost: inventory.averageCost, totalCost: quantity.mul(inventory.averageCost), before: inventory.quantity, after: inventory.quantity.minus(quantity) });
      }
      const totalCost = costs.reduce((sum, item) => sum.plus(item.totalCost), new Prisma.Decimal(0));
      const application = await tx.application.create({ data: { code, lotId: dto.lotId, cropId: dto.cropId, notes: dto.notes?.trim(), appliedAt, totalCost } });
      for (const item of costs) {
        await tx.applicationItem.create({ data: { applicationId: application.id, productId: item.productId, quantity: item.quantity, averageCost: item.averageCost, totalCost: item.totalCost } });
        await tx.inventoryItem.update({ where: { productId: item.productId }, data: { quantity: item.after } });
        await tx.inventoryMovement.create({ data: { productId: item.productId, type: MovementType.APPLICATION, quantity: item.quantity, quantityBefore: item.before, quantityAfter: item.after, unitCost: item.averageCost, referenceType: 'APPLICATION', referenceId: application.id, notes: dto.notes?.trim() } });
      }
      return tx.application.findUniqueOrThrow({ where: { id: application.id }, include: { items: { include: { product: true } }, lot: true, crop: true } });
    });
  }
  async void(id: string, reason: string, actorId?: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const item of application.items) {
        ...
        await tx.inventoryMovement.create({ data: { productId: item.productId, type: MovementType.ADJUSTMENT_IN, quantity: item.quantity, quantityBefore: inventory.quantity, quantityAfter: after, unitCost: item.averageCost, referenceType: 'APPLICATION_VOID', referenceId: application.id, notes: reason.trim() } });
      }
      const result = await tx.application.update({ where: { id }, data: { isVoided: true, voidedAt: new Date(), voidReason: reason.trim() } });

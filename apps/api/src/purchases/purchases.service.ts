  ...
  async void(id: string, reason: string, actorId?: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const item of purchase.items) {
        ...
        if (inventory.quantity.lessThan(item.quantity)) throw new BadRequestException('No se puede anular: parte del inventario de esta compra ya fue consumido.');
        ...
        await tx.inventoryMovement.create({ data: { productId: item.productId, type: MovementType.ADJUSTMENT_OUT, quantity: item.quantity, quantityBefore: inventory.quantity, quantityAfter: after, unitCost: item.unitCost, referenceType: 'PURCHASE_VOID', referenceId: purchase.id, notes: reason.trim() } });
      }
